...
The commands load an input gene expression matrix $\bf{mRNA\_matrix}$,  a data 
frame $\textbf{survData}$ that contains survival information, and an igraph 
object $\textbf{survData}$ that contains the KEGG network constructed by the 
R package $\textbf{iSubpathwayMiner}$.\par
